...
<input formaction="xuqiu_select.action" name="submit" value="查询" type="submit" style="width:60px;height:25px"><br><br>
<script>
function tianjia(){
	for(id=1;id<5;id++){
...
<c:forEach var="item" items="${xuqiuBean1}" varStatus="status">
    		...
    		<td>${item.name}</td>
    		<td><a href="xuqiu_ck.action?xuqiuname=${item.mingcheng}" style="text-decoration:none">${item.mingcheng}</a></td>
    		...
    		<a href="xuqiu_ck.action?xuqiuname=${item.mingcheng}" style="color:blue;text-decoration:none">查看</a>
    		</c:if>
    		<c:if test="${item.status=='已提交'||item.status=='1'}">
    		<a href="xuqiu_delete.action?xuqiuname=${item.mingcheng}" style="color:blue;text-decoration:none">删除</a></td>
    		</c:if>
